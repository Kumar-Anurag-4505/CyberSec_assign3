...
LABEL_COLUMN_CANDIDATES  = ["malware_type", "type", "label", "class", "Category", "family", "Type"]
HASH_COLUMN_CANDIDATES   = ["hash", "md5", "sha256", "Hash", "MD5", "file_hash", "sha1", "file"]
RANSOMWARE_LABEL_VARIANTS = {"ransomware", "Ransomware", "RANSOMWARE", "ransom", "Ransom", "crypto-ransomware", "locker"}

def detect_columns(df: pd.DataFrame):
    label_col, hash_col = None, None
    for c in LABEL_COLUMN_CANDIDATES:
        if c in df.columns: label_col = c; break
    for c in HASH_COLUMN_CANDIDATES:
        if c in df.columns: hash_col = c; break
    non_feat = {label_col, hash_col} - {None}
    api_cols = [c for c in df.columns if c not in non_feat]
    return label_col, hash_col, api_cols
...
def load_virusshare_csv(csv_path: str) -> pd.DataFrame:
    ...
    if 'api' in df.columns and len(df.columns) <= 5:
        print("  [!] Detected 'long' sequence format. Using memory-safe vectorization...")
        file_col = 'file' if 'file' in df.columns else 'hash'
        class_col = 'class' if 'class' in df.columns else 'label'
        
        # Space-separated strings for CountVectorizer
        df['api'] = df['api'].fillna('').astype(str).str.replace(',', ' ')
        
        # Safely extract top 2000 APIs using C-optimized sparse matrices
        vectorizer = CountVectorizer(binary=True, max_features=2000, analyzer=str.split)
        
        print("  [+] Building API matrix...")
        api_matrix = vectorizer.fit_transform(df['api'])
        api_names = vectorizer.get_feature_names_out()
        
        api_df = pd.DataFrame(api_matrix.toarray(), columns=api_names)
        df = pd.concat([df[[file_col, class_col]].reset_index(drop=True), api_df], axis=1)
        print(f"  [+] Expansion complete. Extracted {len(api_names)} unique APIs safely.")

    label_col, hash_col, api_cols = detect_columns(df)
    if label_col is None: raise ValueError("Could not find label column.")

    df["label"]  = df[label_col].apply(lambda x: 1 if str(x).strip() in RANSOMWARE_LABEL_VARIANTS else 0)
    df["family"] = df[label_col].astype(str)

    if hash_col and hash_col in df.columns: df = df.drop(columns=[hash_col])
    if label_col != "label": df = df.drop(columns=[label_col])

    keep_cols = [c for c in api_cols if c not in [hash_col, label_col, "api"]]
    df = df[keep_cols + ["label", "family"]]
